...
public sealed class JpegOptions : LibvipsOptions
{
    ...
    public override void Validate()
    {
        ...
        if (Quality is < 1 or > 100)
            throw new System.ArgumentOutOfRangeException(nameof(Quality), "JPEG quality must be between 1 and 100.");
        if (QuantizationTable is < 0 or > 8)
            throw new System.ArgumentOutOfRangeException(nameof(QuantizationTable));
        if (RestartInterval < 0)
            throw new System.ArgumentOutOfRangeException(nameof(RestartInterval));
    }
}