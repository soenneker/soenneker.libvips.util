...
public sealed class WebpOptions : LibvipsOptions
{
    ...
    public override void Validate()
    {
        ...
        if (Effort is < 0 or > 6)
            throw new System.ArgumentOutOfRangeException(nameof(Effort), "WebP effort must be between 0 and 6.");
        if (AlphaQuality is < 0 or > 100)
            throw new System.ArgumentOutOfRangeException(nameof(AlphaQuality));
        if (MinimumKeyframeDistance < 0)
            throw new System.ArgumentOutOfRangeException(nameof(MinimumKeyframeDistance));
        if (MaximumKeyframeDistance < 0)
            throw new System.ArgumentOutOfRangeException(nameof(MaximumKeyframeDistance));
        if (TargetSize < 0)
            throw new System.ArgumentOutOfRangeException(nameof(TargetSize));
        if (Passes is < 1 or > 10)
            throw new System.ArgumentOutOfRangeException(nameof(Passes));
    }
}