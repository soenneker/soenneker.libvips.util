...
public sealed class TiffOptions : LibvipsOptions
{
    ...
    public override void Validate()
    {
        ...
        if (Quality is < 1 or > 100) throw new System.ArgumentOutOfRangeException(nameof(Quality));
        if (TileWidth is < 1 or > 32768) throw new System.ArgumentOutOfRangeException(nameof(TileWidth));
        if (TileHeight is < 1 or > 32768) throw new System.ArgumentOutOfRangeException(nameof(TileHeight));
        if (BitDepth is not (0 or 1 or 2 or 4 or 8)) throw new System.ArgumentOutOfRangeException(nameof(BitDepth));
        if (!double.IsFinite(XResolution) || XResolution is < 0.001 or > 1_000_000) throw new System.ArgumentOutOfRangeException(nameof(XResolution));
        if (!double.IsFinite(YResolution) || YResolution is < 0.001 or > 1_000_000) throw new System.ArgumentOutOfRangeException(nameof(YResolution));
        if (Level is < 0 or > 22) throw new System.ArgumentOutOfRangeException(nameof(Level));
    }
}
